...
void AbilityItem::Update() {
	...
	if (behaviorRequest_ != Behavior::kUnknown) {
		...
		switch (behavior_) {
		case AbilityItem::Behavior::kDefeated:
		default:
			counter_ = 0;
			break;
		}
	...
	switch (behavior_) {
	case AbilityItem::Behavior::kExist:
...
void AbilityItem::Draw() {
	// 描画
	model_->Draw(worldTransform_, *camera_);
}

Vector3 AbilityItem::GetWorldPosition() {
	// ワールド座標を取得
	Vector3 WorldPos;
...
void AbilityItem::SetNunber(uint32_t number) { AbilityNumber = number; }

AABB AbilityItem::GetAABB() {
