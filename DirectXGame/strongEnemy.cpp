...
void ExplosionEnemy::Initialize(KamataEngine::Model* model, KamataEngine::Camera* camera, const Vector3& position) {
...
Vector3 ExplosionEnemy::GetWorldPosition() {
	// ワールド座標を取得
	Vector3 WorldPos;
...
void ExplosionEnemy::Update() {
	...
	if (behaviorRequest_ != Behavior::kUnknown) {
		...
		switch (behavior_) {
		case ExplosionEnemy::Behavior::kDefeated:
		default:
			counter_ = 0;
			break;
		}
	...
	switch (behavior_) {
	case ExplosionEnemy::Behavior::kWalk:
...
void ExplosionEnemy::CheckMapCollision(CollisionMapInfo& info) {
...
// void ExplosionEnemy::CheckMapCollisionUP(CollisionMapInfo& info) {
//
//	// 上昇あり?
//	if (info.move.y <= 0) {
//		return;
...
void ExplosionEnemy::CheckMapCollisionDown(CollisionMapInfo& info) {
...
Vector3 ExplosionEnemy::CornerPosition(const Vector3& center, Corner corner) {
...
void ExplosionEnemy::UpdateOnWall(const CollisionMapInfo& info) {
...
void ExplosionEnemy::CheckMapCollisionRight(CollisionMapInfo& info) {
...
void ExplosionEnemy::CheckMapCollisionLeft(CollisionMapInfo& info) {
	// 左移動あり?
	if (info.move.x >= 0) {
		return;
	}
...
void ExplosionEnemy::OnCollision(const Player* player) {
	...
	behaviorRequest_ = Behavior::kExplosion;
...
void ExplosionEnemy::OnCollision(const float Level) {
...
void ExplosionEnemy::UpDateOnGround(const CollisionMapInfo& info) {
